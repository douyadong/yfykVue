<template>
    <div class="essays">
        ...
        -->
        <router-link :to="{ path : '/learn/detail/hybrid/' + essay.articleId , query : { openId : pageStates.openId , agentId : pageStates.agentId , cityId : pageStates.cityId } }" class="essay-item top-gap" v-for="( essay , index ) in items" :key="essay.articleId">             
...
<script>     
    export default {
        ...
        data () {            
            return {
                "pageStates" : {
                    "openId" : this.$cookie.get("wxpipOpenId") ,
                    "agentId" : this.$cookie.get("wxpipAgentId") ,
                    "cityId" : this.$cookie.get("wxpipCityId")
                }      
            }
        } ,       
        props : [ "items" ]
